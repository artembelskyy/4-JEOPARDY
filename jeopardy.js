...
async function setupTheGame ()
{
  ...
  {
    let category = await getCategoryData(categoryId);
...
async function getCategoryIds ()
{
  ...
  let validCategories = response.data.filter(category => category.clues_count >= NUMBER_OF_CLUES_PER_CATEGORY);
...
async function getCategoryData (categoryId)
{
  ...
  let shuffledClues = _.sampleSize(response.data.clues, NUMBER_OF_CLUES_PER_CATEGORY);
  for (let clue of shuffledClues)
  {
  ...
  for (let category of categories)
  {
    ...
    for (let clue of category.clues)
    {
      let tr = $("<tr>")
        .text(clue.value)
        .addClass("clue");
      tr.attr("data-category-id", category.id);
      tr.attr("data-clue-id", clue.id);
      tr.on("click", handleClickOfClue);
      td.append(tr);
    }
    $("#clues").append(td);
  }
...
function handleClickOfClue (event)
{
  ...
  let categoryId = $clue.attr("data-category-id");
  let clueId = $clue.attr("data-clue-id");
  let category = categories.find(category => category.id == categoryId);
  let foundClue = category.clues.find(clue => clue.id == clueId);
